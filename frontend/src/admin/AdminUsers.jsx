...
const AdminUsers = () => {
  ...
  return (
    <div style={containerStyle}>
      <h2 style={{ color: '#1DB954', marginBottom: '20px' }}>User Directory</h2>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u._id} style={rowStyle}>
                ...
                <td style={tdStyle}>
                  <span style={{ background: u.role === 'admin' ? 'rgba(29,185,84,0.2)' : 'rgba(16,185,129,0.2)', color: u.role === 'admin' ? '#1DB954' : '#10b981', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 'bold' }}>
                    {u.role.toUpperCase()}
                  </span>
                </td>
